...
export default function CustomAINode({ data }: NodeProps) {
  const d = data as unknown as FlowNodeData;
  const gradeStyle = d.grade ? gradeColors[d.grade] ?? "" : "";
  const borderClass = d.grade ? gradeStyle : "border-violet-400";
  ...
  return (
    <div className={`bg-white border-2 ${borderClass} rounded-lg shadow-sm px-4 py-3 min-w-[180px]`}>
      ...
      <div className="flex items-center justify-between mb-1">
        <span className="text-violet-500 text-xs font-semibold">CUSTOM AI</span>
        {d.grade && (
      ...
      {d.instruction && (
        <div className="text-[10px] text-gray-400 mt-1 truncate max-w-[160px]">
      ...
      {d.outputSchema && d.outputSchema.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-1.5">
          {d.outputSchema.map((field) => (
            <span key={field} className="text-[10px] bg-violet-100 text-violet-700 px-1.5 py-0.5 rounded">
              {field}
            </span>
          ))}
        </div>
      ...
      {d.isRunning && (
        <div className="text-xs text-violet-400 mt-1 animate-pulse">Processing...</div>
      )}
